...
doctl compute droplet create RayTracer --image ubuntu-20-04-x64 --size c-32 --region nyc1 --ssh-keys $key_md5
...
doctl compute ssh RayTracer --ssh-command 'git clone -b raytracing-bookseries https://github.com/antoniogamiz/tfg.git repo && export DO_TOKEN=$DO_TOKEN && cd repo && nohup bash scripts/run-raytracer.sh &'
